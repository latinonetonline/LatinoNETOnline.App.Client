...
namespace LatinoNETOnline.App.Client.Components
{
    ...
    public class AutoSaveEditForm : ComponentBase
    {
        ...
        private async void SaveToStorage(object sender, FieldChangedEventArgs args)
        {
            ...
            await JsRuntime.InvokeVoidAsync($"{GetSelectedStorageType()}.setItem", Id, serializedData);
        }

        private async Task<object> LoadFromStorage()
        {
            var serialisedData = await JsRuntime.InvokeAsync<string>($"{GetSelectedStorageType()}.getItem", Id);
        ...
        private async Task HandleSubmitAsync()
        {
            ...
            {
                await JsRuntime.InvokeVoidAsync($"{GetSelectedStorageType()}.removeItem", Id);
            }
        }
